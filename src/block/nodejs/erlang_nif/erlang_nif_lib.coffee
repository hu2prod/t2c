...
#    erlang_nif_lib
# ###################################################################################################
bdh_module_name_root module, "erlang_nif_lib",
  nodegen       : (root, ctx)->
    ...
    for _k, arch of project_node.data_hash.arch_hash
      arch_name = arch.name
      build2 "erlang_nif", "cd #{folder} && ./build_#{arch_name}.sh"
    false
  ...
  emit_codebub  : (root, ctx)->
    ...
    for _k, arch of project_node.data_hash.arch_hash
      ...
      switch arch.os
        when "linux"
          cb_name = "erlang_nif_lib_#{name}/build_#{arch_name}.sh"
        ...
        when "win"
          cb_name = "erlang_nif_lib_#{name}/build_#{arch_name}.bat"
    ...
    {opt} = root.data_hash
    if opt.url
      cb_name = "erlang_nif_lib_#{name}/clone.sh"
    ...
    for _k, arch of project_node.data_hash.arch_hash
      ...
      switch arch.os
        ...
        when "win"
          ...
          # build2 "erlang_nif", "cd #{folder} && ./build_#{arch_name}.bat"
...
def "erlang_nif_lib", (name, opt, scope_fn)->
  if !mod_runner.current_runner.curr_root
    throw new Error "!current_runner.curr_root"
  
  root = mod_runner.current_runner.curr_root.tr_get "erlang_nif_lib", name, "def"
  bdh_node_module_name_assign_on_call root, module, "erlang_nif_lib"
