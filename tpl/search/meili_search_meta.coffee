...
module.exports = (index_config)->
  ...
  db_attributes = ["id"]
  db_attributes.append field_list
  db ?= require "../db"
  ...
  mod.doc_update_by_id = (doc_id, cb)->
    await mod.get_index defer(err, index); return cb err if err
    await db[table_name].findOne({where:{id:doc_id},attributes:db_attributes,raw:true}).cb defer(err, doc); return cb err if err
    if !doc
  ...
  mod.doc_sync = (_opt, cb)->
    ...
    doc_bn_id_add_list.sort (a,b)->+(a.bn-b.bn).toString()
    ...
    meili_sub_doc_list_push = (cb)->
      ...
      puts "add/update #{total_added}/#{doc_bn_id_add_list.length} #{payload_size} bytes"
    ...
    db_sub_doc_get_push = (cb)->
      return cb() if !db_sub_doc_id_list.length
      
      loc_db_sub_doc_id_list = db_sub_doc_id_list
      db_sub_doc_id_list = []
      
      id_first = loc_db_sub_doc_id_list[0]
      id_last  = loc_db_sub_doc_id_list.last()
      ...
      if +id_last - +id_first <= 2*db_read_batch_size
        ...
        id_dict = new Map
        for id in loc_db_sub_doc_id_list
          id_dict.set id, true
        db_doc_list = []
        ...
        where = {id:loc_db_sub_doc_id_list}
        await db[table_name].findAll({where,attributes:db_attributes,raw:true}).cb defer(err, db_doc_list); return cb err if err
      
      for doc in db_doc_list
    ...
    # DEBUG
    # doc_bn_id_add_list = doc_bn_id_add_list.slice(0, 100)
    
    for v, idx in doc_bn_id_add_list
      doc_id = v.id
      db_sub_doc_id_list.push doc_id
      if db_sub_doc_id_list.length >= db_read_batch_size
  ...
  mod.doc_sync_fast = (_opt, cb)->
    ...
    loop
      ...
      do (meili_doc_list)->
        ...
        await db[table_name].findAll({where,attributes:db_attributes,raw:true}).cb defer(err, db_doc_list); return loc_cb err if err
        # loc_opt = {
        #   table_name
        #   method_name: "findAll"
